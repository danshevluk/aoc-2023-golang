...
func main() {
	...
	fmt.Println("Possible games IDs sum:", solvePart1(inputContent))
	fmt.Println("Sum of powers for min valid sets:", solvePart2(inputContent))
}
...
func solvePart2(inputContent string) int {
	powersOfMinCubeSets := make([]int, 0)
...
Lines:
	for _, line := range strings.Split(inputContent, "\n") {
		...
		// Get game data
		attempts := strings.Split(lineParts[1], ";")
		maxCubeValues := make(map[Cube]int, 0)
		for _, cubesData := range attempts {
			cubesDataParts := strings.Split(cubesData, ",")
			for _, cubeInfo := range cubesDataParts {
				infoItems := strings.Split(strings.TrimSpace(cubeInfo), " ")
				...
				maxCubeValues[cubeName] = max(maxCubeValues[cubeName], count)
			}
		}

		powersOfMinCubeSets = append(
			powersOfMinCubeSets,
			maxCubeValues[Red]*maxCubeValues[Green]*maxCubeValues[Blue],
		)
	}

	result := 0
	for _, item := range powersOfMinCubeSets {
		result += item
	}
	return result
}
